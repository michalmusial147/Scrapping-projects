...
const filePath = "cities/Gdańsk.csv"
const yOffset = 60;
let xMin = 0;
let xMax = 0;
let yMin = 0;
...
function drawCanvas(filePath, canvasId){
  let myStuff = loadFile(filePath);
  myStuff =  myStuff.split(/\r\n|\n/);
  const cityName = myStuff[0];
  const count = myStuff[1];
  const uselessHeader = myStuff[2];
  const cords = myStuff[3].split(",");
  xMin = cords[0];
  xMax = cords[1];
  yMin = cords[2];
  yMax = cords[3];
